...
class CerbosClient:
    ...
    def plan_resources(self, actions: Union[str, List[str]], principal: Principal, resource: ResourceDesc, request_id: Optional[str]=None, aux_data: Optional[AuxData]=None) -> PlanResourcesResponse:
        ...
        """
        req_id = _get_request_id(request_id)
        req = PlanResourcesRequest(request_id=req_id, actions=[actions] if isinstance(actions, str) else list(actions), principal=principal, resource=resource, aux_data=aux_data)
        resp = self._http.post('/api/plan/resources', json={k: v for k, v in req.to_dict().items() if v is not None})
        if resp.is_error:
            if self._raise_on_error:
                raise CerbosRequestException(APIError.from_dict(resp.json()))
            return PlanResourcesResponse(request_id=req_id, status_code=resp.status_code, status_msg=APIError.from_dict(resp.json()), action=actions, resource_kind=resource.kind, policy_version=resource.policy_version)
        data = resp.json()
