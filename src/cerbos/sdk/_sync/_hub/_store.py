...
def _exclude_errors(thrown_type, thrown_value):
    if isinstance(thrown_value, grpc.RpcError):
        ...
        if status is None:
            return False
        return status.code in {code_pb2.ABORTED, code_pb2.CANCELLED, code_pb2.DEADLINE_EXCEEDED}
    return False
    ...
    def list_files(self, store_id: str, filter: Optional[Union[FilterPathEqual, FilterPathContains, FilterPathIn]]=None) -> ListFilesResponse:
        ...
        if filter:
            if isinstance(filter, FilterPathEqual):
                ...
                path_filter = store_pb2.FileFilter(path=store_pb2.StringMatch(contains=filter.fragment))
