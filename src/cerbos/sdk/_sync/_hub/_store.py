...
from cerbos.sdk.hub.store_model import AbortedError, AuthenticationFailedError, CannotModifyGitConnectedStoreError, ChangeDetails, ConditionUnsatisfiedError, File, FileOps, FilterPathEqual, FilterPathIn, FilterPathLike, GetFilesResponse, InvalidRequestError, ListFilesResponse, ModifyFilesResponse, NoUsableFilesError, OperationDiscardedError, PermissionDeniedError, ReplaceFilesResponse, StoreNotFoundError, TooManyFailuresError, UnknownError, ValidationFailureError
    ...
    def wrapper(*args, **kwargs):
        try:
            ...
            if status.code == code_pb2.ABORTED:
                ...
                for detail in status.details:
                    if detail.Is(store_pb2.ErrDetailCannotModifyGitConnectedStore.DESCRIPTOR):
...
class CerbosHubStoreClient(_CerbosHubClientBase):
    ...
    def replace_files(self, store_id: str, message: str, contents: Union[bytes, Iterable[File]], version_must_equal: Optional[int]=None, change_details: Optional[ChangeDetails]=None) -> ReplaceFilesResponse:
        ...
        if isinstance(contents, bytes):
            ...
            req = store_pb2.ReplaceFilesRequest(store_id=store_id, condition=_condition, change_details=_change_details, zipped_contents=contents)
        else:
            files = []
            total_size = 0
            for f in contents:
                ...
                if total_size > _MAX_UPLOAD_SIZE:
                    raise InvalidRequestError(ValueError('total size of files exceeds upload limit'))
                files.append(store_pb2.File(path=f.path, contents=f.contents))
            if len(files) == 0:
                raise InvalidRequestError(ValueError('file list is empty'))
            req = store_pb2.ReplaceFilesRequest(store_id=store_id, condition=_condition, change_details=_change_details, files=store_pb2.ReplaceFilesRequest.Files(files=files))
    ...
    def replace_files_lenient(self, store_id: str, message: str, contents: Union[bytes, Iterable[File]], version_must_equal: Optional[int]=None, change_details: Optional[ChangeDetails]=None) -> ReplaceFilesResponse:
        ...
        """
        try:
            return self.replace_files(store_id, message, contents, version_must_equal, change_details)
        except OperationDiscardedError as e:
            return ReplaceFilesResponse(store_pb2.ReplaceFilesResponse(new_store_version=e.current_store_version))
    ...
    def modify_files(self, store_id: str, message: str, file_ops: FileOps, version_must_equal: Optional[int]=None, change_details: Optional[ChangeDetails]=None) -> ModifyFilesResponse:
        ...
        if file_ops.add:
            ...
            for f in file_ops.add:
                ...
                if total_size > _MAX_UPLOAD_SIZE:
                    raise InvalidRequestError(ValueError('total size of files exceeds upload limit'))
                ops.append(store_pb2.FileOp(add_or_update=store_pb2.File(path=f.path, contents=f.contents)))
        if file_ops.delete:
        ...
        req = store_pb2.ModifyFilesRequest(store_id=store_id, condition=_condition, operations=ops, change_details=_change_details)
    ...
    def modify_files_lenient(self, store_id: str, message: str, file_ops: FileOps, version_must_equal: Optional[int]=None, change_details: Optional[ChangeDetails]=None) -> ModifyFilesResponse:
        ...
        """
        try:
            return self.modify_files(store_id, message, file_ops, version_must_equal, change_details)
        except OperationDiscardedError as e:
            return ModifyFilesResponse(store_pb2.ModifyFilesResponse(new_store_version=e.current_store_version))
    ...
    @handle_store_errors
    @circuitbreaker.circuit(cls=CircuitBreaker)
    def list_files(self, store_id: str, filter: Optional[Union[FilterPathEqual, FilterPathLike, FilterPathIn]]=None) -> ListFilesResponse:
        ...
        if filter:
            if isinstance(filter, FilterPathEqual):
                ...
                path_filter = store_pb2.FileFilter(path=store_pb2.StringMatch(like=filter.pattern))
            elif isinstance(filter, FilterPathIn):
                # "in" is a Python keyword  :(
                path_filter = json_format.ParseDict({'path': {'in': {'values': [p for p in filter.paths]}}}, store_pb2.FileFilter())
        resp = self._store_stub.ListFiles(store_pb2.ListFilesRequest(store_id=store_id, filter=path_filter))
