...
class RetryClient(httpx.Client):
    ...
    def __init__(self, request_retries: int=0, *args, **kwargs):
        ...
        if request_retries:
            ...
            self._post_fn = retry(stop=stop_after_attempt(1 + request_retries), wait=wait_exponential(multiplier=1, min=1, max=10))(fn)
        else:
            self._post_fn = fn
...
class CerbosClient:
    ...
    def __init__(self, host: str, *, timeout_secs: float=2.0, tls_verify: TLSVerify=True, playground_instance: Optional[str]=None, raise_on_error: bool=False, request_retries: int=0, connection_retries: int=0, debug: bool=False, logger: logging.Logger=logging.getLogger(__name__)):
        ...
        self._http = RetryClient(base_url=base_url, headers=headers, timeout=timeout_secs, verify=tls_verify, event_hooks=event_hooks, transport=transport, request_retries=request_retries)
    ...
    def check_resources(self, principal: Principal, resources: ResourceList, request_id: Optional[str]=None, aux_data: Optional[AuxData]=None) -> CheckResourcesResponse:
        ...
        req = CheckResourcesRequest(request_id=req_id, principal=principal, resources=resources.resources, aux_data=aux_data)
        # omit keys with `None` values
        resp = self._http.post('/api/check/resources', json={k: v for k, v in req.to_dict().items() if v is not None})
        if resp.is_error:
    ...
    def is_allowed(self, action: str, principal: Principal, resource: Resource, request_id: Optional[str]=None, aux_data: Optional[AuxData]=None) -> bool:
        ...
        Args:
            ...
            aux_data (None|AuxData): auxiliary data for the request
        """
        resp = self.check_resources(principal=principal, resources=ResourceList().add(resource, {action}), request_id=request_id, aux_data=aux_data)
        if (r := resp.get_resource(resource.id)) is not None:
            return r.is_allowed(action)
        return False

    def plan_resources(self, action: str, principal: Principal, resource: ResourceDesc, request_id: Optional[str]=None, aux_data: Optional[AuxData]=None) -> PlanResourcesResponse:
        ...
        """
        req_id = _get_request_id(request_id)
        req = PlanResourcesRequest(request_id=req_id, action=action, principal=principal, resource=resource, aux_data=aux_data)
        resp = self._http.post('/api/plan/resources', json={k: v for k, v in req.to_dict().items() if v is not None})
        if resp.is_error:
            if self._raise_on_error:
                raise CerbosRequestException(APIError.from_dict(resp.json()))
            return PlanResourcesResponse(request_id=req_id, status_code=resp.status_code, status_msg=APIError.from_dict(resp.json()), action=action, resource_kind=resource.kind, policy_version=resource.policy_version)
...
class PrincipalContext:
    ...
    def plan_resources(self, action: str, resource: ResourceDesc, request_id: Optional[str]=None, aux_data: Optional[AuxData]=None) -> PlanResourcesResponse:
        """Create a query plan for performing the given action on resources of the given kind
