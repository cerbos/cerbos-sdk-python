...
class CerbosAsyncClient:
    ...
    def __init__(
        ...
        *,
        ...
        url = urlparse(host)
        if url.scheme == "unix" or url.scheme == "unix+http":
            transport = httpx.AsyncHTTPTransport(uds=url.path)
            base_url = "http://cerbos.sock"
        elif url.scheme == "unix+https":
            transport = httpx.AsyncHTTPTransport(uds=url.path, verify=tls_verify)
            base_url = "https://cerbos.sock"
